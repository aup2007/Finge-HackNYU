...
import { X, Heart, TrendingUp, TrendingDown, Globe, Users, DollarSign, Building } from "lucide-react"
    ...
    case "AAPL":
      return [
        ...
        },
        {
          ...
          description: "You'll see regular headlines and mood swings in my stock price",
...
export default function StockDetail({ stock, onLike, onPass, status }: StockDetailProps) {
  const variants = {
  ...
  return (
    <AnimatePresence>
      ...
      >
        {status === "liked" && <div className="absolute inset-0 bg-green-500 bg-opacity-50 z-10 rounded-3xl" />}
        {status === "passed" && <div className="absolute inset-0 bg-red-500 bg-opacity-50 z-10 rounded-3xl" />}
